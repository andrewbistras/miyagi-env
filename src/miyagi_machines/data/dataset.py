# /src/miyagi_machines/data/dataset.py
...
def prepare_labels(ds):
    ...
    # Add both labels and labels_model
    ds = ds.map(lambda ex: {
        "labels": 0 if ex["model"]=="human" else 1,
        "labels_model": name2id.get(ex["model"], -100)  # Add this line
    })
...
def get_processed_dataset(
    ...
    if os.path.exists(processed_dataset_path):
        ...
        metadata = {
            "n_models": len([v for v in train_ds.unique("labels_model") if v >= 0]),
            "n_prompts": train_ds.features["labels_prompt"].num_classes,
            "total_size": sum(len(split) for split in processed_ds.values()),
            "splits": {name: len(split) for name, split in processed_ds.items()},
            "label_distribution": dict(Counter(train_ds["labels"])),
            "model_counts": dict(Counter(train_ds["labels_model"])),
            "prompt_counts": dict(Counter(train_ds["labels_prompt"]))
        }
    ...
    metadata = {
        "n_models": n_models,
        "n_prompts": n_prompts,
        "total_size": sum(len(split) for split in ds_dict.values()),
        "splits": {name: len(split) for name, split in ds_dict.items()},
        "label_distribution": dict(Counter(train_ds["labels"])),
        "model_counts": dict(Counter(train_ds["labels_model"])),
        "prompt_counts": dict(Counter(train_ds["labels_prompt"]))
    }
